...
export class ViewModel extends Disposable implements IViewModel {
	...
	constructor(
		...
		private readonly _transactionalTarget: ITransactionalTarget,
	) {
		super();
	...
	private _withViewEventsCollector<T>(callback: (eventsCollector: ViewModelEventsCollector) => T): T {
		return this._transactionalTarget.runInTransaction(() => {
			try {
				const eventsCollector = this._eventDispatcher.beginEmitViewEvents();
...
export interface ITransactionalTarget {
	runInTransaction<T>(cb: () => T): T;
}
